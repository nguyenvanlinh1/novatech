...
export const getReviewByProduct = (reqData) => async (dispatch) => {
    ...
    try{
        const {data} = await api.get(`/reviews/${reqData}`)
...
export const updateReviewByProduct = (reqData, reviewId) => async (dispatch) => {
    dispatch({type: UPDATE_REVIEW_REQUEST})
    try{
        const {data} = await api.put(`/reviews/${reviewId}`, reqData)
